...
static char const* register_name[CFG_REGISTER] = { 
					"IOCFG2",
					"IOCFG1",
					"IOCFG0",
					"FIFOTHR",
...
static Configuration cc1100_GFSK_1_2_kb = {
...
static Configuration cc1100_GFSK_38_4_kb = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration
                    0x80,  // IOCFG0        GDO0 Output Pin Configuration
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds
...
static Configuration cc1100_GFSK_100_kb = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration
                    0x80,  // IOCFG0        GDO0 Output Pin Configuration
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds
...
static Configuration cc1100_MSK_250_kb = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration
                    0x80,  // IOCFG0        GDO0 Output Pin Configuration
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds
...
static Configuration cc1100_MSK_500_kb = {
                    0x07,  // IOCFG2        GDO2 Output Pin Configuration
                    0x2E,  // IOCFG1        GDO1 Output Pin Configuration
                    0x80,  // IOCFG0        GDO0 Output Pin Configuration
                    0x07,  // FIFOTHR       RX FIFO and TX FIFO Thresholds
...
static Configuration  cc1100_OOK_4_8_kb = {
...
///-------------------------------[end]------------------------------------------
void CC1100::goTransmit()
{
    goIdle();
    spi_write_strobe(StrobeCommand::STX);
    while (getState() != ControlState::TX);
}
void CC1100::goIdle()
{
    spi_write_strobe(StrobeCommand::SIDLE);
    while (getState() != ControlState::IDLE);
}

void CC1100::goReceive()
{
    goIdle();
    spi_write_strobe(StrobeCommand::SRX);
    while (getState() != ControlState::RX);
}

uint8_t CC1100::getNumTxBytes()
{
    return spi_read_status(StatusRegister::TXBYTES) & 0x7F;
}

bool CC1100::isRxOverflow()
{
    return (spi_read_status(StatusRegister::RXBYTES) & 0x80) != 0;
}

bool CC1100::isTxUnderflow()
{
    return (spi_read_status(StatusRegister::TXBYTES) & 0x80) != 0;
}

int8_t CC1100::rssiToDbm(uint8_t rawRssi)
{
    if (rawRssi >= 128)
    {
...
void CC1100::reset(void)                  // reset defined in cc1100 datasheet
{
    ...
    spi_write_strobe(StrobeCommand::SRES);
    delay(1);
}
//-----------------------------[END]--------------------------------------------
...
{
    goIdle();
    spi_write_strobe(StrobeCommand::SPWD);               // CC1100 Power Down
}
//-----------------------------[end]--------------------------------------------
...
uint8_t CC1100::begin(volatile uint8_t &My_addr)
{
    ...
    spi_write_strobe(StrobeCommand::SFTX);delayMicroseconds(100);//flush the TX_fifo content
    spi_write_strobe(StrobeCommand::SFRX);delayMicroseconds(100);//flush the RX_fifo content

    partnum = spi_read_status(StatusRegister::PARTNUM); //reads CC1100 partnumber
    version = spi_read_status(StatusRegister::VERSION); //reads CC1100 version number
...
//set ISM band
    ...
    set_output_power_level(0x7);            //set PA to 0dBm as default
    ...
    goReceive();                                  //set CC1100 in receive mode
...
void CC1100::show_register_settings(void)
{
    if(debug_level > 0){
        ...
        {
            printf("%s    \t\t 0x%02X \n", register_name[i], config_reg_verify[i]);
        }
        printf("\r\n");
        printf("PaTable:\r\n");
...
ControlState CC1100::getState()
{
    ControlState res = (ControlState)spi_read_status(StatusRegister::MARCSTATE);
    //printf("state:%d\n",(uint8_t)res);
    return res;
}
...
*/
    ...
    spi_write_register(Register::MCSM0, 0x18);    //FS Autocalibration
    spi_write_register(Register::MCSM2, 0x01);    //MCSM2.RX_TIME = 1b

    // configure EVENT0 time
    spi_write_register(Register::WOREVT1, 0xFF);  //High byte Event0 timeout
    spi_write_register(Register::WOREVT0, 0x7F);  //Low byte Event0 timeout

    // configure EVENT1 time
    spi_write_register(Register::WORCTRL, 0x78);  //WOR_RES=0b; tEVENT1=0111b=48d -> 48*(750/26MHz)= 1.385ms

    spi_write_strobe(StrobeCommand::SFRX);             //flush RX buffer
    spi_write_strobe(StrobeCommand::SWORRST);          //resets the WOR timer to the programmed Event 1
    spi_write_strobe(StrobeCommand::SWOR);             //put the radio in WOR mode when CSn is released
...
{
    goIdle();                            //exit WOR Mode
    spi_write_register(Register::MCSM2, 0x07);    //stay in RX. No RX timeout
}
//-------------------------------[end]------------------------------------------
...
uint8_t CC1100::tx_payload_burst2(uint8_t my_addr, uint8_t rx_addr,
...
void CC1100::writeTxFifo(uint8_t *data, uint8_t length)
{
    spi_write_burst(TXFIFO_BURST, data, length);
}
...
uint8_t CC1100::rx_payload_burst(uint8_t rxbuffer[], uint8_t &pktlen)
{
    ...
    bytes_in_RXFIFO = spi_read_status(StatusRegister::RXBYTES);              //reads the number of bytes in RXFIFO
    ...
    else
    {
        ...
        goIdle();                                                  //set to IDLE
        spi_write_strobe(StrobeCommand::SFRX);delayMicroseconds(100);            //flush RX Buffer
        goReceive();                                                //set to receive mode
        res = FALSE;
    }
                            ...
                            uint8_t pktlen,  uint8_t tx_retries)
{
    ...
    do                                                          //sent package out with retries
    {
        ...
        printf("sendb\n");
        goTransmit();                                             //sents data over air
        printf("sendc\n");
        goReceive(); 
        printf("sendd\n");                                             //receive mode
...
void CC1100::sent_acknowledge(uint8_t my_addr, uint8_t tx_addr)
{
    ...
    goTransmit();                                                 //sent package over the air
    goReceive();                                                  //set CC1100 in receive mode
...
uint8_t CC1100::packet_available()
{
    ...
    {
        if(spi_read_register(Register::IOCFG2) == 0x06)               //if sync word detect mode is used
        {
    ...
    else
    {
        ...
        else                                               //real data, and sent acknowladge
        {
            ...
            if(debug_level > 0){                           //debug output messages
                ...
                printf("RX_FIFO: (%d)", pktlen);
                for(uint8_t i = 0 ; i < pktlen + 1; i++)   //showes rx_buffer for debug
                {
                    printf("0x%02X ", rxbuffer[i]);
                }
                printf("| RSSI(0x%02X) LQI(0x%02X) |", rxbuffer[pktlen+1], rxbuffer[pktlen+2]);
                printf("\r\n");
...
{
    spi_write_register(Register::ADDR,addr);          //stores MyAddr in the CC1100
}
//-------------------------------[end]------------------------------------------
...
{
    spi_write_register(Register::CHANNR,channel);   //stores the new channel # in the CC1100
...
void CC1100::set_mode(uint8_t mode)
{
    ...
    {
        case 0x01:
                    writeConfiguration(&cc1100_GFSK_1_2_kb);
                    break;
        case 0x02:
                    writeConfiguration(&cc1100_GFSK_38_4_kb);
                    break;
        case 0x03:
                    writeConfiguration(&cc1100_GFSK_100_kb);
                    break;
        case 0x04:
                    writeConfiguration(&cc1100_MSK_250_kb);
                    break;
        case 0x05:
                    writeConfiguration(&cc1100_MSK_500_kb);
                    break;
        case 0x06:
                    writeConfiguration(&cc1100_OOK_4_8_kb);
                    break;
        case 0x07:
					printf("p2a:%d\n", mode);
                    writeConfiguration(&cc1100_ideo);
                    break;
        default:
                    writeConfiguration(&cc1100_GFSK_100_kb);
                    break;
...
void CC1100::writeConfiguration(Configuration *config)
{
    spi_write_burst(0,(uint8_t *)config,sizeof(Configuration));
...
void CC1100::set_ISM(uint8_t ism_freq)
{
    ...
    spi_write_register(Register::FREQ2,freq2);                                         //stores the new freq setting for defined ISM band
    spi_write_register(Register::FREQ1,freq1);
    spi_write_register(Register::FREQ0,freq0);
...
void CC1100::set_output_power_level(int8_t dBm)
{
    ...
    spi_write_register(Register::FREND0,pa);
}
//-------------------------------[end]------------------------------------------
...
{
    uint8_t data;
    data = spi_read_register(Register::MDMCFG2);
    data = (data & 0x8F) | (((cfg) << 4) & 0x70);
    spi_write_register(Register::MDMCFG2, data);
...
{
    uint8_t data;
    data = spi_read_register(Register::MDMCFG1);
    data = (data & 0x8F) | (((cfg) << 4) & 0x70);
    spi_write_register(Register::MDMCFG1, data);
    //printf("MDMCFG2: 0x%02X\n", data);
}
//-------------------------------[end]-----------------------------------------
...
{
    spi_write_register(Register::MDMCFG4, mdmcfg4);
    spi_write_register(Register::MDMCFG3, mdmcfg3);
    spi_write_register(Register::DEVIATN, deviant);
}
//-------------------------------[end]-----------------------------------------
...
{
    uint8_t data;
    data = spi_read_register(Register::PKTCTRL0);
    data = (data & 0xBF) | (((cfg) << 6) & 0x40);
    spi_write_register(Register::PKTCTRL0, data);
...
uint8_t CC1100::get_temp(uint8_t *ptemp_Arr)
{
    ...
    if(debug_level > 0){
        printf(F("Temp:"));printf(ptemp_Arr[0]);printf(F("."));printf(ptemp_Arr[1]);
    }
...
void CC1100::spi_begin(void)
{
	...
	//pinMode(SCK_PIN, ALT0);
    //pinMode(MOSI_PIN, ALT0);
    //pinMode(MISO_PIN, ALT0);
    ...
    digitalWrite(SS_PIN, HIGH);
	 ...
	 int x = 0;
     printf ("init SPI bus... ");
     if ((x = wiringPiSPISetup (SPI_CHIP_SELECT, 100000)) < 0)  //4MHz SPI speed
     {
          if(debug_level > 0){
...
//------------------[write register]--------------------------------
void CC1100::spi_write_register(Register spi_instr, uint8_t value)
{
     uint8_t tbuf[2] = {0};
     tbuf[0] = ((uint8_t)spi_instr) | WRITE_SINGLE_BYTE;
     tbuf[1] = value;
     uint8_t len = 2;
     wiringPiSPIDataRW (SPI_CHIP_SELECT, tbuf, len) ;

     return;
}
//|============================ Ein Register lesen ============================|
uint8_t CC1100::spi_read_register(Register spi_instr)
{
	return spi_read_register_i((uint8_t)spi_instr);
}

uint8_t CC1100::spi_read_status(StatusRegister spi_instr)
{
	return spi_read_register_i((uint8_t)spi_instr);
}

uint8_t CC1100::spi_read_register_i(uint8_t spi_instr)
{
     uint8_t value;
     uint8_t rbuf[2] = {0};
     rbuf[0] = ((uint8_t)spi_instr) | READ_SINGLE_BYTE;
     uint8_t len = 2;
     wiringPiSPIDataRW (SPI_CHIP_SELECT, rbuf, len) ;
     value = rbuf[1];
...
void CC1100::spi_write_strobe(StrobeCommand spi_instr)
{
     uint8_t tbuf[1] = {0};
     tbuf[0] = (uint8_t)spi_instr;
     //printf("SPI_data: 0x%02X\n", tbuf[0]);
     wiringPiSPIDataRW (SPI_CHIP_SELECT, tbuf, 1) ;
 }
...
void CC1100::spi_read_burst(uint8_t spi_instr, uint8_t *pArr, uint8_t len)
{
     ...
     wiringPiSPIDataRW (SPI_CHIP_SELECT, rbuf, len + 1) ;
...
void CC1100::spi_write_burst(uint8_t spi_instr, uint8_t *pArr, uint8_t len)
{
     ...
     wiringPiSPIDataRW (SPI_CHIP_SELECT, tbuf, len + 1) ;
}
//|================================= END =======================================|


uint8_t CC1100::parseNibble(char param)
{
  uint8_t nib;
  if (param >= '0' && param <= '9')
    nib = param - '0';
...
uint16_t CC1100::parseUint16(const char *param)
{
  uint16_t retval = 0;
...
void CC1100::printTxPacket(const RawPacket *packet)
{
...
void CC1100::printPacket(const RawRxPacket *packet)
{
  uint16_t param_1 = parseUint16(packet->params);
  uint16_t param_2 = parseUint16(&packet->params[4]);
  ...
  {
    case 0x21:
        printf("Unknow command : 0x21\n");
        break;

    case 0x31:
        printf("Get Outlet? Temperature: \n");
        printf("   Sondes de l’extraction air vicié : %f\n",param_1/10.00);
        printf("   Sondes entrée air neuf           : %f\n",param_2/10.00);
        break;

    case 0x32:
        printf("Get Outside Temperature: \n");
        printf("   Température extérieure : %f \n",param_1/10.00);
        break;
    ...
    case 0x58:
        printf("Set Contact Polarity\n");
        break;

    case 0x59:
        printf("Dirty filter alarm threshold\n");
        if (param_2 == 0)
        {
            printf("Value (RPM): ");
            printf("%d\n", param_1);
...
void CC1100::printParams(const char *params)
{
  for (int i = 0; i < 8; i++)
    printf("%d ", params[i]);
}
...
uint16_t CC1100::computeChecksum(const RawPacket *pkt)
{
  uint8_t chk = 0;
  chk += pkt->header >> 8;
  chk += pkt->header & 0xff;
...
void CC1100::sendPacket(TxPacketData *packet)
{
  RawPacket txPacket;
  ...
  printTxPacket(&txPacket);
  if (getNumTxBytes())
    spi_write_strobe(StrobeCommand::SFTX);

  writeTxFifo((uint8_t *)&txPacket, sizeof(RawPacket));
  spi_write_strobe(StrobeCommand::STX);
